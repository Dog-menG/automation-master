...
try:
    ...
    try:
        path = os.getcwd()
        if path[-6:] == 'AP711T' or path[-4:] == 'Velo' or path[-6:] == 'DSS203':
            os.chdir('..')
...
class AAReadWrite(object):
    ...
    def __init__(self, port, i2c_add, pullup=False):
        ...
        if type(i2c_add) is str:
            ...
            if self.i2c_add | 1000000 > 127:
                self.log.warning("[I2C]: Aardvark Only use 7bit I2C address rather than 8bit\n"
                                 "e.g. D4 is 0x6a, self calibrated to 7bit address")
        elif type(i2c_add) is int:
            self.i2c_add = i2c_add
        self.pullup = pullup
        self.handle = aa_open(port)
        if self.handle <= 0:
            self.log.error("[I2C]: Unable to open Aardvark device on port %d" % port)
            self.log.error("[I2C]: Error code = %d" % self.handle)
            self.log.error("[I2C]: Exiting now")
            sys.exit()
        self.aa_init()
    ...
    def aa_init(self):
        ...
        self.log.info("[I2C]: Bitrate set to %d kHz" % self.bitrate)

        # Set the bus lock timeout
        bus_timeout = aa_i2c_bus_timeout(self.handle, self.bus_timeout)
        self.log.info("[I2C]: Bus lock timeout set to %d ms" % bus_timeout)

    def aa_write_i2c(self, str_addr, wrt_lst):
        """
        Write I2C content, starts with a register address
        :param str_addr: register start address, takes string at hex format or integer
        :param wrt_lst: a list of content bytes that has to write
        """
    ...
    def aa_read_i2c(self, str_addr, length=None):
        """
        I2C read function
        :param str_addr: Starts register address, writing with a start address then starts reading
        :param length: definable length, fixed
        ...
        if count < 0:
            self.log.error("[I2C]: error: %s" % aa_status_string(count))
            raise ValueError
        elif count == 0:
            self.log.error("[I2C]: no bytes read")
            self.log.error("please check device I2C address settings")
            raise ValueError
        elif count != self.length:
            self.log.error("[I2C]: read %d bytes (expected %d)" % (count, self.length))
            raise ValueError
